...
public class testCadenas {
    ...
    public Cola generar(Cola c1) {
        // Orden O(n^2) recorro la cola por bloques y por cada dato del nodo apilo en
        // una pila e inserto en una lista
        ...
        while (balanceo && !q.esVacia()) {
            ...
            if (c == '{' || c == '[' || c == '(') { // si es una apertura la apilo
                ...
                if (p.esVacia()) {
                    ...
                    // si el tope que es una apertura es igual a c que deberia su cerradura entonces
                    // desapila, sino está desbalanceada
                    if ((c == ')' && tope == '(') || (c == ']' && tope == '[') ||
                            (c == '}' && tope == '{')) {
    ...
    public Lista generarSecuencia(Cola q, int t) {
        Lista l = new Lista(); // lista resultante
        Cola aux = new Cola(); // cola auxliar
        Cola copia = q.clone(); // cola que uso para recorrer
        Pila p = new Pila(); // pila auxiliar
        int cont = 1; // contadora auxiliar
        int i = 1;
        while (!copia.esVacia()) {
            Object d = copia.obtenerFrente();
            copia.sacar();
            ...
            if (cont % t == 0) {
                ...
                while (!aux.esVacia()) {
                    l.insertar(aux.obtenerFrente(), i);
                    aux.sacar();
                    i++;
                }
                l.insertar('$', i);
    ...
    public void testGenerarSecuencia() {
        ...
        Lista l = generarSecuencia(q, 4);

        assertEquals("[3,2,1,0,0,1,2,3,$,7,6,5,4,4,5,6,7,$,9,8,8,9]", l.toString());
